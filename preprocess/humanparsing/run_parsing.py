...
class Parsing:
    def __init__(self, gpu_id: int, human_parsing_model_path):
        self.gpu_id = gpu_id
        ...
        self.session = ort.InferenceSession(os.path.join(Path(__file__).absolute().parents[2].absolute(), human_parsing_model_path+"/parsing_atr.onnx"),
                                            sess_options=session_options, providers=['CPUExecutionProvider'])
        self.lip_session = ort.InferenceSession(os.path.join(Path(__file__).absolute().parents[2].absolute(), human_parsing_model_path+"/parsing_lip.onnx"),
                                                sess_options=session_options, providers=['CPUExecutionProvider'])
